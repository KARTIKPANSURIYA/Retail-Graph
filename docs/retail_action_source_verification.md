...
## Hardening status (2026-09-25)

The current `main` still contains **no RetailAction source converter**, pinned commit SHA, trusted
validation digest, verified metadata fixture, or `inspection_subset.json` implementation. The task
request referred to these as existing, but repository inspection confirmed they are absent. The
local `data/raw/retail_action/data/validation.tar` is also absent. Official metadata access remains
blocked by the proxy described above.

Because camera keys, camera metadata fields, TAR member layout, video linkage, `labels.action`,
spatial-view keys, and internal split fields have not been directly verified, this change does not
invent validators or a converter for them. A preflight command now supplies only source-independent
guarantees: chunked SHA-256, manifest digest lookup, filename/`--split` agreement, fail-closed
benchmark behavior, audit reporting, and removal of stale benchmark artifacts. Even a matching
digest stops at `blocked_unverified_source_schema` until the ingestion gate is satisfied.

The manifest contains explicit `null` digest entries for train, validation, and test. These are
fail-closed sentinels, not pins. Replace `validation.tar`'s `null` only with the digest obtained from
the immutable official repository manifest at the pinned revision; never copy a digest calculated
from an untrusted local download into the trusted manifest without independent verification.
